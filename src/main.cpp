...
// 'main' function.
// Cats doesn't require any arguments for now.
int main(int argc, char** argv) {
  std::cout << "Crypto Arbitrage Trading System (CATS)" << std::endl;
...
// would be useful to collect historical bid/ask data.
  ...
  // We need at least two exchanges to run Cats
  if (index < 2) {
    std::cout << "ERROR: Cats needs at least two Bitcoin exchanges. Please edit the config.json file to add new exchanges\n" << std::endl;
    exit(EXIT_FAILURE);
  }
  // Creates the CSV file that will collect the trade results
  std::string currDateTime = printDateTimeFileName();
  std::string csvFileName = "output/cats_result_" + currDateTime + ".csv";
  ...
  std::string logFileName = "output/cats_log_" + currDateTime + ".log";
  ...
  // Log file header
  logFile << "-------------------------------------------------------" << std::endl;
  logFile << "|   Crypto Arbitrage Trading System (CATS) Log File   |" << std::endl;
  logFile << "------------------------------------------------------\n" << std::endl;
  logFile << "Cats started on " << printDateTime() << "\n" << std::endl;
  ...
  for (int i = 0; i < numExch; ++i) {
    ...
    if (balance[i].leg1 > 0.0050 && !inMarket) { // FIXME: hard-coded number
      logFile << "ERROR: All " << params.leg1 << " accounts must be empty before starting Cats" << std::endl;
      exit(EXIT_FAILURE);
    }
  }
  logFile << std::endl;
  ...
  while (stillRunning) {
    ...
    // at the end, so Cats is not stopped by default.
