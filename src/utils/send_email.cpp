...
void sendEmail(const Result &res, Parameters &params) {
  ...
  oss.precision(2);
  oss << std::fixed;
  oss << "sendemail -f " << params.senderAddress << " -t " << params.receiverAddress << " -u \"Crypto Arbitrage Trading System (CATS) - Trade " << res.id <<" (";
  ...
  oss << "      <caption style=\\\"" << captionStyle << "\\\">BCrypto Arbitrage Trading System (CATS) - Trade " << res.id << "</caption>";
